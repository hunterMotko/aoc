...
func RunDay10Part1() {
	start := time.Now()
	defer PrintTimeElapsed(start, "This")
	maze := make([]string, 0)
	startPos := Pos{
		x: 0,
		y: 0,
...
func RunDay10Part2() {
	...
	cwPath = append(cwPath, Pipe{
		x: cwPos.x, y: cwPos.y,
		pipe: maze[cwPos.y][cwPos.x], dist: dist,
	})
	dist++
	for mazeRunner {
		if mazeRunner {
			...
			if nextCw2.Equals(lastCwPos) {
				...
				pipe := Pipe{
					x: nextCw1.x, y: nextCw1.y,
					pipe: maze[nextCw1.y][nextCw1.x], dist: dist,
				}
				cwPath = append(cwPath, pipe)
				lastCwPos = cwPos
				cwPos = nextCw1
				...
				pipe := Pipe{
					x: nextCw2.x, y: nextCw2.y,
					pipe: maze[nextCw2.y][nextCw2.x], dist: dist,
				}
				cwPath = append(cwPath, pipe)
				lastCwPos = cwPos
				cwPos = nextCw2
